...
resource "aws_instance" "app" {
  ...
  user_data = <<-EOF
    ...
    mkdir -p /opt/deployment_home/www

    curl -SL https://github.com/docker/compose/releases/latest/download/docker-compose-linux-$(uname -m) \
       -o /usr/libexec/docker/cli-plugins/docker-compose
    sudo chmod +x /usr/libexec/docker/cli-plugins/docker-compose

    cat > /opt/deployment_home/config.json <<'CONFIGEOF'
    ${file("${path.module}/../../config-docker.json")}
    CONFIGEOF

    cat > /opt/deployment_home/engine.json <<'ENGINEEOF'
    ${file("${path.module}/../../engine.json")}
    ENGINEEOF

    cat > /opt/deployment_home/app-compose.yaml <<'COMPOSEEOF'
    ${file("${path.module}/../../deploy/app-compose.yaml")}
    COMPOSEEOF

    cat > /opt/deployment_home/www/index.html <<'INDEXEOF'
